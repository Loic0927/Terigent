...
import { validateLogin, validateRegistration } from './user-validation.js';
...
export function createUserAuthHandlers(repository, clock = Date.now) {
  ...
  async function me(req,res){res.setHeader('Cache-Control','private, no-store');if(req.method!=='GET'){res.setHeader('Allow','GET');return send(res,405,{error:'Method not allowed.'});} const token=readUserToken(req); if(!token)return send(res,401,{error:'Sign-in is required.'}); try{const user=await repository.findSession(hashSessionToken(token));return user?send(res,200,{user}):send(res,401,{error:'Sign-in is required.'});}catch(error){console.error('User session lookup failed:',error instanceof Error?error.message:'Unknown error');return safeError(res);}}
